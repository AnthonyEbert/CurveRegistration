...
In statistical problems of interest, what constitutes the sample space can sometimes be a matter of perspective. For instance, in spectroscopy, if we take the spectra of a series of samples then we will have a measurement for each combination of frequency and sample. The sample space could be the range of the measurement (0 to 1), or, alternatively, the set of all possible spectra could be the sample space, which is a function space. In functional data analysis (FDA), we take the latter approach and focus on problems where this perspective provides useful insight [@hsing2015theoretical,@ramsay2006functional]. 

Data representing these problems are termed functional data. A common problem when analysing functional data is that these data are misaligned, if data are misaligned it can be difficult to compare observations or to derive meaningful statistics. Alignment is not always achieved with a simple shift of the time axis, sometimes a more complex warping of this time axis is required. Common statistical methods assume that only the amplitude of a function is subject to noise, however it is possible to consider these warpings of the time axis as another type of noise, which must be accounted for. @Kneip2008 elegantly terms this as separation of amplitude and phase variation within a functional dataset. @srivastava2011registration extended this paradigm by developing a distance, with mathematically convenient properties, between functions displaying amplitude and phase variation. Such distances, which take into account both sources of variability, are termed elastic distances.  
