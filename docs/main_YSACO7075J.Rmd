...
The FR metric (Equation \@ref(eq:FR)) is a dissimilarity, another dissimilarity which we use is an estimator for maximum mean discrepancy (MMD). We refer to the estimator for MMD simply as MMD for the sake of brevity. MMD was developed by @gretton_kernel_2007 as a non-parametric two-sample statistic for testing whether samples come from the same distribution. @park2016k2 use MMD as a dissimilarity within an ABC algorithm. In our case, $\mathbf{x}$ and $\mathbf{y} \sim q$ represent functional datasets which we label as $f$ and $g$ respectively. The MMD estimator is:
\begin{align}
...
\end{align}
The model is an input/output model like the airport example, however, in this case, the input observation $f^a$ and the latent input variable are one and the same, so we do not include a corrected loss. We use the same loss functions as were used in the peak shifted example (Algorithm \@ref(alg:loss) and \@ref(alg:regloss)). 
