...
#' @export
simulator_sGaussian <- function(Time, param = c(1, 0.01), alpha = c(10, 21, 25, 40, 50), basel = 0, mean_global = 0, sigma_a = 5){

  sigma_global  <- param[1]
  sigma_e       <- param[2]

  mean_global   <- ifelse(is.na(param[3]), mean_global, param[3])
  sigma_a       <- ifelse(is.na(param[4]), sigma_a, param[4])
  ...
  for(i in 1:length(Time)){
    y[i] <- sum(dnorm(Time[i], mean = a + alpha, sd = sigma_global)) + rnorm(1, 0, sigma_e) + basel * Time[i]
  }
