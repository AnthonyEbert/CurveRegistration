...
```{r}
load("runs/sGaussian/test/ABC_sG_5.RData")
...
```{r, message=FALSE}
load(file = "runs/airport/test/ABC_airport.RData")
...
other_df <- data.frame(value = c(1, 1), registration = c(TRUE, FALSE))

vline_df <- vline_df %>% left_join(other_df)
...
```{r}
load("runs/hydrology/test/ABC_hydro_syn.RData")
