...
output_tidy$parameter[which(output_tidy$parameter == "rho[phi]")] <- "sigma[C]"

# output_tidy$parameter <- factor(output_tidy$parameter, labels = c("rho[phi]", "sigma[epsilon]", "sigma[phi]"))

true_params = c(1, 0.01, 0.7)

vline_df = data.frame(parameter = c("sigma[phi]", "sigma[epsilon]","sigma[C]"), input = true_params, value = c(1, 1, 1))

blank_df <- data.frame(parameter = c("sigma[phi]", "sigma[epsilon]","sigma[C]"), value = c(3,0.02,2))
...
output_tidy$parameter <- factor(output_tidy$parameter, levels = c("sigma[phi]", "sigma[C]", "sigma[epsilon]"))
...
out <- ggplot(output_tidy2) +
  aes(x = value, linetype = united, color = united) +
...
ggsave(filename = "sGaussianPost-2.pdf", width = 13, height = 6, units = "cm", plot = out, scale = 1.5)
